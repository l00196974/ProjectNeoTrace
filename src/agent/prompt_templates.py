...
{{
  ...
  "confidence": <0.0-1.0 的浮点数，表示整体分析的置信度>
}}

置信度说明：
- high: 有明确的行为证据支持该意图（如深度浏览、多次访问、长时间停留）
- medium: 有一定行为证据但不够充分（如简单浏览、短暂停留）
- low: 仅有轻微迹象或间接证据

整体置信度（confidence）说明：
- 0.9-1.0: 非常确定，行为模式清晰明确
- 0.7-0.9: 比较确定，有充分的行为证据
- 0.5-0.7: 中等确定，行为证据较为模糊
- 0.0-0.5: 不太确定，行为证据不足或矛盾
...
{{
  "intents": [
    {{"name": "automotive_comparison", "confidence": "high", "reasoning": "深度浏览多款车型配置"}},
    {{"name": "automotive_financing", "confidence": "medium", "reasoning": "查看金融贷款页面"}},
    {{"name": "automotive_purchase", "confidence": "medium", "reasoning": "对比车型并关注金融方案"}},
    {{"name": "general_food_delivery", "confidence": "high", "reasoning": "使用美团点外卖"}}
  ],
...
注意：
1. 只输出 JSON，不要有其他文字
2. confidence（意图级别）必须是 "high"、"medium" 或 "low" 之一
3. confidence（整体级别）必须是 0.0-1.0 之间的浮点数
4. urgency_score 只针对 automotive_purchase 意图，其他情况设为 0
5. 如果没有明确的汽车相关意图，urgency_score 设为 0
"""
